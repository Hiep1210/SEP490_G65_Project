...
onMounted(async () => {
  if (paypalButton.value) {
    const options: PayPalScriptOptions = {
      clientId:
        'AfA7cxuOdrPU6GvcxtuYqCNYrO_k2EcgtZhsI5Kl3Z_0r3_0lIHy1JymnEYuR31tGFqr2-lEIulHsTRL', // Replace with your actual client ID
      currency: 'USD'
    }
      ...
      paypal
        .Buttons({
          ...
          onApprove: async (data: OnApproveData, actions: OnApproveActions) => {
            ...
            const transactionId = (await actions.order.capture()).id
            const isDeposit = props.status === 'IN_PROGRESS';


            await confirmPayment(props.clientId, transactionId as string, props.orderId, isDeposit)
