...
namespace verbum_service_test.Impl.Service
{
    ...
    public class ReceiptServiceImplTests
    {
        ...
        public async Task CreateReceiptTest()
        {
            ...
            var request = new CreateReceipRequest()
            {
                OrderId = Guid.Parse("551cc0f7-4600-4b69-a07f-44b7817b3e30"),
                DepositeOrPayment = false,
                Amount = 300
            };

            mockMapper.Setup(m => m.Map<Receipt>(It.IsAny<CreateReceipRequest>()))
                      .Returns(new Receipt
                      {
                          OrderId = Guid.Parse("551cc0f7-4600-4b69-a07f-44b7817b3e30"),
                          DepositeOrPayment = false,
            ...
            //Act
            receiptServiceImpl.CreateReceipt(request);

            //Assert
            var addedReceipt = await dbContext.Receipts.FirstOrDefaultAsync(c => c.OrderId == Guid.Parse("551cc0f7-4600-4b69-a07f-44b7817b3e30") && c.DepositeOrPayment == false && c.Amount == 300);
            Assert.IsNotNull(addedReceipt);
        }

        [TestMethod]
        public async Task CreateReceiptTest_Exception()
        {
            //Arrange
            var mockMapper = new Mock<IMapper>();
