...
<template>
  <Dialog :open="isOpen" @click-outside="closeDialog" @close="closeDialog">
    <DialogContent class="h-4/6 max-w-[425px]">
      <DialogHeader>
        <DialogTitle class="text-cyan-600 font-bold"> Payment Detail for {{ order.orderName }}</DialogTitle>
        <DialogDescription
        ...
        <div>
          ...
          <div class="flex">
            <div class="flex-none w-1/2">
              <p class="my-2 font-semibold">Service:</p>
            </div>
            <div class="flex-auto">
                <p v-if="order.hasTranslateService" class="my-2 "
                  ><span class="font-bold">TRN - </span>Translate
                </p>
                <p v-if="order.hasEditService" class="my-2 "><span class="font-bold">EDIT - </span> Edit </p>
                <p v-if="order.hasEvaluateService" class="my-2 ">
                  <span class="font-bold">EVA - </span>Evaluate
                </p>
            </div>
          </div>
          <hr>
          <div class="flex mb-2">
            <div class="flex-none w-1/2">
              <p class="my-2 font-semibold">Source Language:</p>
              <p class="my-2 font-semibold">Target Language:</p>
            </div>
            <div class="flex-auto">
          ...
          <div class="flex">
            <div class="flex-none w-1/2">
              <p class="my-3 font-semibold">Total prices:</p>
              <p
                class="my-1 font-semibold"
                :class="
                  status === 'IN_PROGRESS' ? ' my-3 font-bold text-xl  text-cyan-600' : ''
                "
              >
                Deposit ({{ deposit * 100 }}%):
              </p>
